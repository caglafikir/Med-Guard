...
def run_inference(model, X_train_t, X_cal_t, X_test_t,
    ...
    neg_lambda_inf = cfg.NEG_LAMBDA if cfg.NEG_LAMBDA_INFER is None else cfg.NEG_LAMBDA_INFER

    model.eval()
    with torch.no_grad():
        H_train_local, logits_train_local, _ = model(X_train_t)
        ...
        if cfg.INFER_PROP:
            ...
            H_cal_prop, H_test_prop = H_cal_local, H_test_local
        ...
        H_train_final = H_train_pos_final - cfg.NEG_LAMBDA * H_train_neg_delta
        logits_train_prop, _ = model.forward_from_features(H_train_final)
